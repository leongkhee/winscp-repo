...
try
{
    ...
    try
    {
        ...
        $synchronizationResult = $session.SynchronizeDirectories(
            [WinSCP.SynchronizationMode]::Local, $localPath, $remotePath, $false,[WinSCP.SynchronizationCriteria]::Time)
        ...
        foreach ($Download in $synchronizationResult.Downloads)
        {
            ...
            {
               "Download of {1} succeeded" -f (Get-Date), $Download.FileName  | logger -logFileName $logfile -WriteHost:$true
            }
            else
            {
           
               "Download of {1} failed: {2}" -f (Get-Date), $Download.FileName, $Download.Error.Message  | logger -logFileName $logfile -WriteHost:$true -errortype "Error"
            }
        }
    }
    finally
